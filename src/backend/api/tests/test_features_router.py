...
class TestPostUpvote:
    def test_creates_upvote_and_user_row(self, client, db_conn):
        _insert_feature(db_conn, "f1")
    ...
    def test_idempotent_double_post(self, client, db_conn):
        _insert_feature(db_conn, "f1")
...
class TestDeleteUpvote:
    def test_removes_upvote(self, client, db_conn):
        _insert_feature(db_conn, "f1")
    ...
    def test_idempotent_double_delete(self, client, db_conn):
        _insert_feature(db_conn, "f1")
        client.post("/api/features/f1/upvote")
    ...
    def test_delete_without_prior_upvote_is_idempotent(self, client, db_conn):
        _insert_feature(db_conn, "f1")
        resp = client.delete("/api/features/f1/upvote")
        assert resp.status_code == 200
...
class TestResolveUserIdForMutationErrorBranches:
    ...
    def test_missing_sub_claim_returns_401(self, test_app, db_conn):
        _insert_feature(db_conn, "f1")
    ...
    def test_missing_email_claim_returns_401(self, test_app, db_conn):
        _insert_feature(db_conn, "f1")
        saved = test_app.dependency_overrides.get(get_current_user)
        test_app.dependency_overrides[get_current_user] = lambda: {
...
class TestListFeaturesNoUserRow:
    ...
    user has no row in ``users`` yet. The endpoint should succeed and report
...
class TestListFeaturesDbErrorRollback:
    ...
    branch in `list_features` (routers/features.py). If the SELECT raises,
    the pooled connection must be rolled back (else the next caller sees
    'current transaction is aborted') and the endpoint must return 500.
    """
...
class TestResolveOptionalUserIdDbErrorFallback:
    ...
    (routers/features.py). When `get_user_by_email` raises, the router must:
    (a) roll back the connection so the transaction isn't aborted for
