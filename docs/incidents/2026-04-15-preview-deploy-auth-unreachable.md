...
2. **Fundamental gap** — even if we fixed the crash and uploaded the real auth env vars to Preview, auth still couldn't *function* on previews because:
   - Preview URLs are dynamic (`Careerbase-git-<branch>-<hash>.vercel.app`).
...
- `vercel.json` → replace the literal `Access-Control-Allow-Origin` with a reflective origin check in each serverless function (`api/*.ts`), allowlisting `*-careerbase-projects.vercel.app` plus the prod domain.
- Auth0 → add `https://*-careerbase-projects.vercel.app` to callback/logout/web-origin lists.
