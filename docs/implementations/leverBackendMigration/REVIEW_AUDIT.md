...
**Verifier results:**
- `vercel-prod-verifier` (inline checks via repo state): Vercel project `Careerbase` configuration. PR deletes `api/lever.ts` + the `/api/lever/:path(.*)` rewrite in `vercel.json`. No env-var changes required (Lever's public Postings API needs no auth). Auto-deploys on merge.
