...
- Replace `Access-Control-Allow-Origin: "*"` in `vercel.json` with the actual production Vercel domain (`https://Careerbase.vercel.app`). The current wildcard allows any website to make API requests to the backend.
...
- **Type:** `spa`
- **Allowed Callback URLs:** `http://localhost:3000, https://Careerbase.vercel.app`
- **Allowed Logout URLs:** `http://localhost:3000, https://Careerbase.vercel.app`
- **Allowed Web Origins:** `http://localhost:3000, https://Careerbase.vercel.app` (required for silent token renewal)
...
- **Identifier (audience):** `https://Careerbase.vercel.app/api`
...
Set three new variables on the `Careerbase` service:

| Variable | Value |
|----------|-------|
| `AUTH0_DOMAIN` | `dev-mbnkjr1sc4ccwlup.us.auth0.com` |
| `AUTH0_AUDIENCE` | `https://Careerbase.vercel.app/api` (from Step 2) |
| `GOOGLE_CLIENT_ID` | Same value as `VITE_GOOGLE_CLIENT_ID` from `.env.local` |

Existing `CORS_ORIGINS=https://Careerbase.vercel.app` is already correct.
...
| `VITE_AUTH0_REDIRECT_URI` | `https://Careerbase.vercel.app` |
| `VITE_AUTH0_AUDIENCE` | `https://Careerbase.vercel.app/api` (from Step 2) |
...
1. Add `https://Careerbase.vercel.app` to **Authorized JavaScript origins**
