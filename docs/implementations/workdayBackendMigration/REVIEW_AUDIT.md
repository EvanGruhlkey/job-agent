...
- `vercel-prod-verifier` (repo state): Vercel project `Careerbase` configuration. PR deletes `api/workday.ts` + the `/api/workday/:path(.*)` rewrite in `vercel.json` + the `X-Workday-Base-Url` CORS allow-header. No env-var changes required (Workday's CXS endpoint needs no auth). Auto-deploys on merge.
