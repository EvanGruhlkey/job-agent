...
- Set `ANTHROPIC_API_KEY` on the Railway `Careerbase` service (feature dormant until then; auto-recovers after).
...
- **Manual action before activation:** set `ANTHROPIC_API_KEY` on Railway service `Careerbase` (feature dormant + safe until then; auto-recovers after).
