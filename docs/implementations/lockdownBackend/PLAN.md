...
- **Anonymous browsing must keep working** on `careerbase.local` — visitors should still see jobs without signing in. The lockdown must be transparent to the browser, which means a **server-side shared secret** between the Vercel proxy layer and the Railway backend.
...
This kills direct scraping of `https://Careerbase-production.up.railway.app/api/jobs` without changing how the browser talks to the app, and gives a clean future hook for issuing per-customer API keys.
...
1. **Direct backend scraping is blocked**:
   ```bash
   curl -sI https://Careerbase-production.up.railway.app/api/jobs
   # expect: HTTP/2 401
   curl -s https://Careerbase-production.up.railway.app/api/jobs | head
   # expect: {"detail":"Unauthorized"}
   ```

2. **Health check still works** (for Railway probe + external monitors):
   ```bash
   curl -s https://Careerbase-production.up.railway.app/health
   # expect: OK
   ```

3. **Frontend still works via Vercel proxy**:
   - Visit `https://careerbase.local` (or `https://Careerbase.vercel.app`) in a browser.
...
- **Does not respond to the stranger's email.** That's the maintainer's call (decline, ignore, or have a conversation).
